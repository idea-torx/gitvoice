...
function renderCoverPage(invoice: InvoiceDraft | InvoiceRecord): string {
  ...
  const contact = clientContactName(invoice.client);
...
export function clientContactName(client: InvoiceDraft["client"]): string {
  return [client.contactFirstName, client.contactLastName].map((part) => (part || "").trim()).filter(Boolean).join(" ");
}
